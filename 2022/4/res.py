...
LogischerEinschlussGruppe1=np.logical_and(PaarMatrix[:,0]<=PaarMatrix[:,2], PaarMatrix[:,1]>=PaarMatrix[:,3])
# Prüfe Einschluss der ersten Gruppe seitens der zweitens Gruppe 
LogischerEinschlussGruppe2=np.logical_and(PaarMatrix[:,2]<=PaarMatrix[:,0], PaarMatrix[:,3]>=PaarMatrix[:,1])
